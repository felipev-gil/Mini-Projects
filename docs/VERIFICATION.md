...
- **181 exact-case local HTML links**, 26 active HTML pages and 28 application JavaScript files passed the static audit. All 25 catalog URLs returned HTTP 200 under `/Mini-Projects/`.
...
Keyboard interaction and viewport emulation are browser checks, not a full screen-reader audit or tests on physical phones. Animated effects honor `prefers-reduced-motion`. The original examples were removed only after their integrated versions passed the recorded checks.
